...
// ==================== Inserting Location Visa  ---------------------------------------//
function Register_Exam_Catgories($conn)
{
	$data = array();
	extract($_POST);
	// SELECT `rowNo`, `Ex_Cat_ID`, `Ex_Cat_Name`, `Ex_Cat_Description`, `RegisteredBy`, `RegisterDate`, `UpdatedBy`, `UpdateDate` FROM `exam_catagory` WHERE 1
	// $agency_id=1;
	foreach ($_POST['txtCatagoryName'] as $key => $catagory_name) {
		$description = $_POST['txtCatagoryDescription'][$key];
		// $agency_id = $_POST['txtagencyid'];
		$registerDate = $_POST['txtRegisterDate'];
		$txtUserrIddd = $_POST['txtRegisterUser'];
		$getMaxID = mysqli_query($conn, "SELECT MAX(rowNo) FROM exam_catagory") or die(mysqli_error($conn));
		if (mysqli_num_rows($getMaxID) > 0) {
			$maxID = mysqli_fetch_array($getMaxID);
			$curMaxID = $maxID[0] + 1;
			$generatedID = "EXC" . $curMaxID . "-" . date("Y");
		} else {
			$curMaxID = 1;
			$generatedID = "EXC" . $curMaxID . "-" . date("Y");
		}
		$query = "SELECT  * FROM `exam_catagory` WHERE `Ex_Cat_Name`='$catagory_name' and `Ex_Cat_Description`='$description' ";
		$q = mysqli_query($conn, $query);
		if ($q->num_rows > 0) {
			$data = array("status" => false, "Message" => "This Catagory already Saved");
		} else {
			// SELECT `rowNo`, `Ex_Cat_ID`, `Ex_Cat_Name`, `Ex_Cat_Description`, `RegisteredBy`, `RegisterDate`, `UpdatedBy`, `UpdateDate` FROM `exam_catagory` WHERE 1
			$sql = "INSERT INTO exam_catagory VALUES('','$generatedID','$catagory_name','$description','$txtUserrIddd','$registerDate','-','-')";
			$q = mysqli_query($conn, $sql);
			if ($q) {
				$data = array('Status' => True, 'Message' => "This Catagory(s) has been registered successfully");
			} else {
...
// ====================================== Fill Doctor's Information =======================================
function fill_contactDoctor($conn)
{

	$contact_person_data = array();
	$data = array();

	$ContactPersonIdd = mysqli_query($conn, "SELECT * FROM Doctors") or die(mysqli_error($conn));
	if ($ContactPersonIdd) {
		$num_rows = mysqli_num_rows($ContactPersonIdd);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_array($ContactPersonIdd)) {
				$contact_person_data[] = $row;
			}
			$data = json_encode(array('Status' => true, 'Message' => $contact_person_data));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Doctors Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
...
function PatientDoctorAppo($conn)
{
	$patients = array();
	$doctor_id = 'DOC2-2022';
	$data = array();
	$result = mysqli_query($conn, "SELECT appointments.appointment_id,patients.patient_name,patients.patient_tell,doctors.doctor_name,appointments.appointment_number,appointments.appointment_status FROM appointments JOIN patients ON (patients.patient_id = appointments.patient_id) JOIN doctors ON doctors.doctor_id=appointments.doctor_id WHERE appointments.appointment_status=0 and doctors.doctor_id='$doctor_id'  ORDER BY appointments.appointment_number ASC");
	if ($result) {
		$num_rows = mysqli_num_rows($result);
		if ($num_rows > 0) {
			while ($row = mysqli_fetch_assoc($result)) {
			...
			$data = json_encode(array('Status' => true, 'Message' => $patients));
		} else {
			$data = json_encode(array('Status' => false, 'Message' => "Patient's Apointment Not Found"));
		}
	} else {
		$errorMsg = mysqli_error($conn);
		$data = json_encode(array('Status' => false, 'Message' => $errorMsg));
	...
	<div class="tile">
		<div class="tile-body">
			<div class="container-fluid">
				...
				<div class="row mb-2">
					<div class="col-md-12">
						<h3 class="text-uppercase">Baaritaanada Bukaanka</h3>
					</div>
				</div>
				<div class="row">
					<div class="col-md-12">
						<table class="table table-striped" id="grandUserWebpagesToVisitTable">
							<?php
							$getMain = mysqli_query($conn, "SELECT DISTINCT exam_catagory.Ex_Cat_ID,exam_catagory.Ex_Cat_Name FROM exam_catagory GROUP by exam_catagory.Ex_Cat_Name ORDER BY exam_catagory.Ex_Cat_Name ");
							if ($getMain) {
								$main_num = mysqli_num_rows($getMain);
								if ($main_num > 0) {
									while ($main_rs = mysqli_fetch_array($getMain)) {
							?>
										<tr>
											<td style="padding: 10px 0px 10px 10px;">
												<?php if ($main_rs[0] == "1") { ?>
													<input type="checkbox" checked="checked" class="form-control" style="height: 17px; width: 17px; display: inline;" name="allowedLabCatagory" id="allowedLabCatagory" data-id="<?php echo $main_rs[0]; ?>" data-src="<?php echo $currentAppointment; ?>"> &nbsp;&nbsp;
												<?php } else { ?>
													<input type="checkbox" class="form-control" style="height: 17px; width: 17px; display: inline;" name="allowedLabCatagory" id="allowedLabCatagory" data-id="<?php echo $main_rs[0]; ?>" data-src="<?php echo $currentAppointment; ?>"> &nbsp;&nbsp;
												<?php } ?>
												<i class="<?php echo $main_rs[2]; ?>"></i> &nbsp;&nbsp;<a href="<?php echo $main_rs[3]; ?>"><b><?php echo $main_rs[1]; ?></b></a>
											</td>
										</tr>
										<tr>
											<td>
												<div class="row">
													<?php
													$getSubMain = mysqli_query($conn, "SELECT DISTINCT  exam_catagory.Ex_Cat_ID  , exam_catagory.Ex_Cat_Description FROM exam_catagory  WHERE exam_catagory.Ex_Cat_Name='" . $main_rs[1] . "'");
													...
													if ($getSubMain) {
														...
														if ($submenu_num > 0) {
															while ($submenu_rs = mysqli_fetch_array($getSubMain)) {
																...
																<div class="col-md-4" style="padding: 10px 10px 10px 75px;">
																	<?php if ($submenu_rs[0] == "1") { ?>
																		<input type="checkbox" checked="checked" class="form-control" style="height: 17px; width: 17px; display: inline;" name="allowedLab" id="allowedLab" data-id="<?php echo $submenu_rs[0]; ?>" data-src="<?php echo $currentAppointment; ?>"> &nbsp;&nbsp;
																	<?php } else { ?>
																		<input type="checkbox" class="form-control" style="height: 17px; width: 17px; display: inline;" name="allowedLab" id="allowedLab" data-id="<?php echo $submenu_rs[0]; ?>" data-src="<?php echo $currentAppointment; ?>"> &nbsp;&nbsp;
																	<?php } ?>
																	<i class="<?php echo $submenu_rs[2]; ?>"></i> &nbsp;&nbsp;<a href="<?php echo $submenu_rs[3]; ?>"><?php echo $submenu_rs[1]; ?></a>
																</div><?php
																	}
		...
		<div class="tile-footer" style="text-align: right;">
			<button type="submit" class="btn btn-primary" name="regUser" id="regUser"><i class="fa fa-fw fa-lg fa-save"></i>Register Examr</button>&nbsp;&nbsp;&nbsp;
